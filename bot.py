...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Если уже был запуск — игнорируем
    if context.user_data.get("start_called"):
        logging.info(f"🔁 Повторный запуск /start от {user_id}, игнорируем.")
        return
    ...
    intro = (
        ...
        "_Анкета хранится 30 дней, чтобы не перегружать сервер. Спасибо за понимание!_"
    )
